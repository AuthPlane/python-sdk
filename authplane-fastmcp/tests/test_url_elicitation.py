...
def test_url_elicitation_builds_valid_params_under_installed_mcp() -> None:
    # The consent-driven path must yield a genuine, schema-valid
    # ``ElicitRequestURLParams`` with a fresh id under the installed mcp.
    error = ConsentRequiredError(
        "user must grant access",
        service_id="calendar",
        cause_detail="missing_user_consent",
    ...
    UUID(elicitation["elicitationId"])
    rebuilt = ElicitRequestURLParams.model_validate(elicitation)

"""DPoP context forwarding + per-request verify cache tests.

Covers two coupled changes:

* ``verify_token`` must build a ``DPoPRequestContext`` from the active
  HTTP request and forward it to ``AuthplaneResource.verify`` so the
  proof check actually runs. Without it, a resource configured for
  ``inbound_dpop=InboundDPoPOptions(required=True)`` silently accepts
  bearer-only requests even though PRM advertises the requirement.
* Repeated ``verify_token`` calls within the same HTTP request must
  collapse onto a single underlying ``verify`` so the inbound replay
  store cannot see the same proof ``jti`` twice within one request.
  Distinct requests get distinct caches so cross-request replay
...
def _make_token_verifier(
    ...
    that ``fastmcp.server.dependencies.get_http_request`` raises outside a
    request context.
    """
    ...
    return AuthplaneTokenVerifier(
        mock_resource,
        base_url="https://api.example.com",
        get_http_request=fake_get_http_request,
    )
...
async def test_dpop_context_forwarded_to_verify() -> None:
    ...
    Before this change the bare ``verify(token)`` call left
    ``dpop_request`` at its default ``None``, which the core verifier
    treats as "no proof presented" — so any ``inbound_dpop=required=True``
    configuration silently accepted bearer-only requests.
    """
    mock = _mock_verifier()
...
async def test_no_http_request_context_falls_back_to_no_dpop() -> None:
    ...
    fastmcp.server.dependencies.get_http_request() raises RuntimeError
    when called without an active request — the adapter must verify
    without a DPoP context rather than throw.
    """
    mock = _mock_verifier()
...
async def test_cache_is_scoped_per_request() -> None:
    ...
    verifier = AuthplaneTokenVerifier(
        mock,
        base_url="https://api.example.com",
        get_http_request=pop_request,
    )
...
# ---------------------------------------------------------------------------
# Constructor and request-lookup guards (review hardening)
# ---------------------------------------------------------------------------
...
def test_init_rejects_non_string_resource() -> None:
    ...
    type(mock).scopes = PropertyMock(return_value=[])

    with pytest.raises(TypeError, match=r"verifier\.resource must be a str URI"):
        AuthplaneTokenVerifier(mock, base_url="https://api.example.com")
...
async def test_unrelated_runtimeerror_from_get_http_request_propagates() -> None:
    ...
    A future FastMCP release surfacing an unrelated ``RuntimeError`` from
    ``get_http_request`` (context manager not entered, etc.) must not be
    masked as ``dpop_request=None`` — that would silently re-introduce
    the no-proof-forwarded silent-pass this module fixed.
    """
    mock = _mock_verifier()

    def broken_get_http_request() -> Request:
        raise RuntimeError("FastMCP internal: context not initialized")

    verifier = AuthplaneTokenVerifier(
        mock,
        base_url="https://api.example.com",
        get_http_request=broken_get_http_request,
    )
